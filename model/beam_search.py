...
class BeamSearch:
    ...
    def search(self, start_predictions, start_state, dec_rnn_h, step):
        """
        ...
        last_context=torch.zeros((batch_size,1,dec_rnn_h)).to('cuda')
        start_class_log_probabilities, state, last_context = step(
            start_predictions, start_state, last_context)
        ...
        #print('\nstart predict',start_predicted_classes.shape)
        #print('first context vector',last_context.shape)
        ...
        last_context_=torch.zeros((batch_size*self.beam_size,1,dec_rnn_h)).to('cuda')
        for bas in range(batch_size):
            for bes in range(self.beam_size):
                idx=bas*self.per_node_beam_size+bes
                last_context_[idx]=last_context[bas]
        last_context=last_context_
        ...
        for timestep in range(self.max_steps - 1):
            ...
            #print('last context before input',last_context.shape)
            class_log_probabilities, state, last_context = step(last_predictions, state, last_context)
            #print('class log probability',class_log_probabilities.shape)
            #print('last context after input',last_context.shape)
            ...
            for bas in range(batch_size):
                for bes in range(self.beam_size):
                    idx=bas*self.per_node_beam_size+bes
                    last_context_[idx]=last_context[bas*self.per_node_beam_size+restricted_beam_indices[bas][bes]//self.per_node_beam_size]
            last_context=last_context_
        ...
        for timestep in range(len(predictions) - 2, 0, -1):
            ...
            cur_preds = predictions[timestep].gather(
                1, cur_backpointers.long()).unsqueeze(2)

            reconstructed_predictions.append(cur_preds)

            # shape: (batch_size, beam_size)
            cur_backpointers = backpointers[timestep -
                                            1].gather(1, cur_backpointers.long())
