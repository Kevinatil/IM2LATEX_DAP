...
class Im2LatexModel(nn.Module):
    def __init__(self, out_size, emb_size, dec_rnn_h,
        ...
        self.W_add=nn.Linear(dec_rnn_h,enc_out_dim,bias=False) # transform for the last context vector
    ...
    def forward(self, imgs, formulas, epsilon=1.):
        ...
        logits = []
        last_context_t=torch.zeros((imgs.shape[0],1,self.dec_rnn_h)).to('cuda')
        for t in range(max_len):
            ...
            # ont step decoding
            dec_states, o_t, logit, last_context_t = self.step_decoding(dec_states, o_t, encoded_imgs, tgt,last_context_t)
            logits.append(logit)
    ...
    def step_decoding(self, dec_states, o_t, enc_out, tgt, last_context_t):
        ...
        # context_t : [B, C]
        context_t, attn_scores = self._get_attn(enc_out, h_t,last_context_t)
        ...
        return (h_t, c_t), o_t, logit, context_t.unsqueeze(1)

    def _get_attn(self, enc_out, h_t, last_context):
        """Attention mechanism
        args:
        ...
        #print('last_context',last_context.shape)
        #print('h_t',h_t.shape)
        #print('enc_out',enc_out.shape)
        alpha=torch.tanh(self.W_1(enc_out)+self.W_2(h_t).unsqueeze(1)+self.W_add(last_context))#attention mechanism with last context vector
