...
class LatexProducer(object):
    ...
    def __init__(self, model, vocab, dec_rnn_h, beam_size=5, max_len=64, use_cuda=True):
        """args:
    ...
    def _greedy_decoding(self, imgs):
        ...
        with torch.no_grad():
            for t in range(self.max_len):
                dec_states, O_t, logit,last_context_t = self.model.step_decoding(
                    dec_states, O_t, enc_outs, tgt,last_context_t)
    ...
    def _batch_beam_search(self, imgs):
        ...
        all_top_k_predictions, log_probabilities = self._beam_search.search(
            start_predictions, state,self.dec_rnn_h, self._take_step)
    ...
    def _take_step(self, last_predictions, state, last_context):
        ...
        with torch.no_grad():
            dec_states, O_t, logit, last_context = self.model.step_decoding(
                dec_states, O_t, enc_outs, last_predictions, last_context)

        # update state
        state['h_t'] = dec_states[0]
        state['c_t'] = dec_states[1]
        state['o_t'] = O_t
        return torch.log(logit), state, last_context
